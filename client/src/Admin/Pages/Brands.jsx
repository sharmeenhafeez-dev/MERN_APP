...
export default function Brands() {
  ...
  const deleteBrand = (BrandName) => {
    axios
      .delete(`${AppRoute}api/delete-brand`, {
        data: { BrandName },
      })
      .then((response) => {
        setBrand(response.data.updatedBrands);
  ...
  const saveEdits = () => {
    ...
    axios
      .put(`${AppRoute}api/update-brand`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      })
  ...
  return (
    <div > 
      <div className='d-flex flex-column flex-lg-row align-items-lg-center justify-content-between mt-3'>
        <h2 className='mb-3 mb-lg-0 p-3'>Brand</h2>
        <AddBrands recallData={setBrand} />
      </div>
        ...
        <table className='table ' >
          <thead   >
            <tr >
              ...
              <th scope='col'className='bg-dark text-white'>Brand Image</th>
          ...
          <tbody >
            {brand?.map((val, index) => (
              <tr  key={val._id}>
                <td style={{ backgroundColor: 'peachpuff' }}>{index + 1}</td>
